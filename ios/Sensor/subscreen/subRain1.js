...
import { AppRegistry, StyleSheet, Text, View, TouchableOpacity,Animated, Dimensions,ScrollView,ActivityIndicator } from 'react-native';
...
export default class SubRain1 extends Component {
  ...
  getone = () =>{
    ...
    fetch(global.url+'sun')
    .then(response => response.json())
    .then((responseJson)=> {
      this.setState({
       dataSource2: responseJson.rowz,
      })
      this.setState({
        rain:this.state.dataSource2.map(element => element.be),
      })
      ...
      <SafeAreaView style={{flex:1,}}>
        ...
        <Animated.View style={{justifyContent:'center', alignItems:'center',flex:1,flexDirection:'row'}}>
                <Animated.View style={{flex:1, justifyContent:'center', alignItems:'center',}}>
                            ...
                            {
                                (fill)=>(
                                    <Text style={{fontSize:25, color:this.state.rain>100 ? 'red':this.state.rain>95 ? 'black':'blue'}}>
                                        {this.state.rain}
                                    </Text>
