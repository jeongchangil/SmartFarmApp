...
export default class SubTemp2 extends Component {
  ...
  getData = () =>{
   ...
   .then((responseData) => {
    ...
    this.setState({dataSource:responseData.rowt})
    //이미지 url을 dataSource에 받아옴
    console.log(this.state.dataSource)
    this.setState({humData:this.state.dataSource.map(dataSource=>dataSource.hum)})
    this.setState({tempData:this.state.dataSource.map(dataSource=>dataSource.temp)})
  ...
  render() {
    ...
    return (
      ...
      <Animated.View style={{flex:1,transform:[{scale:this.state.chart2}]}}>
            <LineChart
            data={{
            labels: ["0시", "3시", "6시", "9시", "12시", "3시","6시","9시"],
            datasets: [
