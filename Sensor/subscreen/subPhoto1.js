...
export default class SubPhoto1 extends Component {
  ...
  getone = () =>{
    ...
    .then((responseJson)=> {
      ...
      this.setState({
        photo :this.state.dataSource2.map( element=>element.bit),
      })
        ...
        <Animated.View style={{justifyContent:'center', alignItems:'center',flex:1,flexDirection:'row'}}>
                <Animated.View style={{flex:1, justifyContent:'center', alignItems:'center',}}>
                            ...
                            >
                            {
                                ()=>(
                                  <View>
                                    <Text style={{fontSize:25, color:this.state.photo>100 ? 'red':this.state.photo>95 ? 'black':'blue'}}>
                                      {this.state.photo}
                                    </Text>
                                    <Text style={{fontSize: 18,textAlign: 'center'}}>Lux</Text>
                                  </View>
                                )
                            }
                            </AnimatedCircularProgress>
